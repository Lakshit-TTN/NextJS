...
export const getStaticProps = async ({ params }:{ params: { id: string } }) => {
    ...
    return {
        props:{ product:data },
        revalidate:10 //updates after 10 seconds
    };
}
//to use getStaticPaths we need to defines getStaticProps
export const getStaticPaths = async ()=>{
    const res = await fetch(`https://dummyjson.com/products/`);
    const data = await res.json();
    const paths = data.products.map((product: ProductCardInterface) => ({
        params: { id: product.id.toString() },//as we need string to make routes for each id
    }));
    return{
        paths,
        fallback:false //returns 404 for other routes
    }
}
